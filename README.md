...
AIアシスタント（Claude等）からdevDashのプロジェクト・タスク・メンバー・マイルストーン・ステータスを操作するための[MCP (Model Context Protocol)](https://modelcontextprotocol.io)サーバーです。
...
- スコープは用途に応じて選択
  - `tasks:read/write` — タスクの参照・操作
  - `projects:read/write` — プロジェクトの参照・操作
  - `members:read/write` — メンバーの参照・操作
  - `milestones:read/write` — マイルストーンの参照・操作
  - `statuses:read/write` — ステータスの参照・操作
...
```
devDashのプロジェクト一覧を見せて
```

MCPサーバーが正しく接続されていれば、`projects_list` ツールが呼び出されます。
...
| ツール名 | 機能 | 必要なスコープ |
|---------|------|--------------|
| `projects_list` | プロジェクト一覧取得（フィルタ・ページネーション対応） | `projects:read` |
| `projects_get` | プロジェクト詳細取得 | `projects:read` |
| `projects_create` | プロジェクト作成 | `projects:write` |
| `projects_update` | プロジェクト部分更新 | `projects:write` |
| `projects_delete` | プロジェクト削除（アーカイブ） | `projects:write` |

### メンバー

| ツール名 | 機能 | 必要なスコープ |
|---------|------|--------------|
| `members_list` | プロジェクトメンバー一覧取得 | `members:read` |
| `members_get` | メンバー詳細取得 | `members:read` |
| `members_add` | プロジェクトにメンバー追加 | `members:write` |
| `members_update` | メンバー情報更新 | `members:write` |
| `members_remove` | プロジェクトからメンバー削除 | `members:write` |

### マイルストーン

| ツール名 | 機能 | 必要なスコープ |
|---------|------|--------------|
| `milestones_list` | マイルストーン一覧取得 | `milestones:read` |
| `milestones_get` | マイルストーン詳細取得 | `milestones:read` |
| `milestones_create` | マイルストーン作成 | `milestones:write` |
| `milestones_update` | マイルストーン部分更新 | `milestones:write` |
| `milestones_delete` | マイルストーン削除 | `milestones:write` |

### ステータス

| ツール名 | 機能 | 必要なスコープ |
|---------|------|--------------|
| `statuses_list` | タスクステータス一覧取得 | `statuses:read` |
| `statuses_get` | ステータス詳細取得 | `statuses:read` |
| `statuses_create` | ステータス作成 | `statuses:write` |
| `statuses_update` | ステータス部分更新 | `statuses:write` |
...
- 「プロジェクト一覧を見せて」
- 「プロジェクトXXXのメンバー一覧を見せて」
- 「プロジェクトXXXのマイルストーンを確認して」
- 「"ログイン画面のバグ修正"というタスクをプロジェクトXXXに作って、優先度はhighで」
- 「タスクXXXの期限を来週金曜に変更して」
- 「新しいマイルストーン"フェーズ2リリース"を作成して」
- 「新しいステータス"レビュー中"を作成して、カテゴリはin-progressで」
- 「メンバーXXXのアサイン率を80%に更新して」
...
### "devDash API error: Scope xxx:write is required"

- APIキーに必要なスコープが付与されているか確認してください
